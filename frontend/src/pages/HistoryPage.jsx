  ...
  useEffect(() => {
    const fetchHistory = async () => {
      try {
        ...
        // In a real app, you would call getHistory()
    ...
    <div className="max-w-4xl mx-auto">
      ...
      {history.length === 0 ? (
        <div className="bg-white rounded-xl shadow-sm p-6 border border-gray-100 text-center py-12">
          ...
          <p className="text-gray-600 mb-6">Your processed content will appear here once generated</p>
          <button
            onClick={handleBack}
        ...
        <div>
          {history.map((item, index) => (
            <div key={index} className="bg-white rounded-xl shadow-sm p-6 border border-gray-100 mb-6">
              <div className="flex items-center justify-between mb-4">
                <div>
                  <h3 className="font-medium text-gray-900">
                    {item.type === 'text' ? 'Text Input' : 
                     item.type === 'file' ? `File: ${item.input}` : 
                     `URL: ${item.input.substring(0, 40)}${item.input.length > 40 ? '...' : ''}`}
                  </h3>
